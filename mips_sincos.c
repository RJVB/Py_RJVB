...
#if defined(__APPLE_CC__) || defined(__MAC__)
#	define BUILD_OS_DARWIN
#endif
...
#pragma weak mips_sincos = mips__sincos
#endif

#if defined(BUILD_OS_DARWIN) /* Mach-O doesn't support aliases */ || defined(__CYGWIN__) || defined(linux)
	static void mips__sincos(double, double *, double *);
#	ifndef __CYGWIN__
#		pragma weak mips_sincos
#	endif
	void mips_sincos(double x, double *sinx, double *cosx) {
	  return mips__sincos(fmod(x, (M_PI+M_PI)), sinx, cosx);
	}
//#elif defined(__CYGWIN__)
//#	define	mips__sincos	mips_sincos
#elif defined(__GNUC__)
extern  void  mips__sincos(double, double *, double *);

void    mips_sincos() __attribute__ ((weak, alias ("mips__sincos")));
...
/* ====================================================================
 *
 * FunctionName    mips__sincos
 *
 * Description    computes sin(arg) , cos(arg)
 *
 * Note:  Routine mips__sincos() computes sin(arg) and cos(arg) and should
...
static void
mips__sincos(double x, double *sinx, double *cosx)
{
#ifdef _32BIT_MACHINE
int	ix, xpt, m, l;
#else
...
#if defined(BUILD_OS_DARWIN) /* Mach-O doesn't support aliases */ || defined(__CYGWIN__)
static void mips__sincosl(long double, long double *, long double *);
void mips_sincosl(long double x, long double *sinxl, long double *cosxl) {
  double sinx, cosx;

   mips__sincos((double)x, &sinx, &cosx);
...
void    mips_sincosl() __attribute__ ((weak, alias ("mips__sincosl")));

#endif

static void
mips__sincosl(long double x, long double *sinxl, long double *cosxl)
{
double	sinx, cosx;
...
main()
{ double s,c;
	printf( "half=%g one=%g Qnan=%g\n", half.d, one.d, Qnan.d );
	printf( "sin(2)=%g, cos(2)=%g\n", sin(2), cos(2) );
	mips_sincos(2, &s, &c);
	printf( "sincos(2)=%g , %g\n", s, c );
	printf( "diff sin,cos-sincos: %g , %g\n", sin(2)-s, cos(2)-c );
}
